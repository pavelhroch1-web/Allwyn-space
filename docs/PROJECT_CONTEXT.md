...
### 7.1 Tržby → appka generuje Tourplan (dlouhodobá vize, rozhodnuto 2026-06-21)

Dnes appka Tourplan jen **importuje** (externí zdroj rozhoduje kdo/kam/kdy). Pavel chce
v budoucnu dodávat **tržby per POS** a appka by měla Tourplan sama **generovat** —
převzít roli dnešního externího procesu/systému. Tohle je kvalitativně jiná úloha než
dnešní RouteEngine (ten jen optimalizuje pořadí v rámci už hotového přiřazení; tady by
šlo o rozhodování o přiřazení samotném).

Odsouhlasený postup — **nepřeskakovat kroky**:

- **A — Tržby jako vstup do Task poolu (první krok, až Pavel dodá data).**
  Tržby naplní `salesValue`/`priorityScore` v POS modelu (`js/posModel.js`, dnes
  placeholder `null`). Vznikne "Coverage Intelligence" — appka upozorní Velína
  ("POS X má vysoké tržby, ale N týdnů nenavštíven"), Velín to ručně přidá do
  existujícího Task poolu (Tourplan / Service request / Kampaň / Ad hoc POS list →
  Task pool → Route Intelligence → Plánování technika — viz `CLAUDE.md`). Tourplan
  import se nemění.
- **B — Shadow mode.** Appka postaví optimizer (frekvence návštěv + kapacita technika +
  region) a generuje "návrh Tourplanu" vedle reálně importovaného — Velín porovnává,
  appka se nepoužívá k ničemu produkčnímu. Vyžaduje, aby Pavel dodal byznys pravidla:
  **podle čeho se určuje, jak často má která POS dostat návštěvu** (tržby? kanál?
  smluvní frekvence? kombinace?) — bez tohoto pravidla by appka musela hádat, což je
  zakázané (žádná fake operační realita).
- **C — Plná náhrada Tourplanu.** Appka rozhoduje, Velín jen schvaluje/publikuje —
  nikdy automatické publikování technikům bez schválení (stejná filozofie jako
  "asistent, ne dohled" u technika, jen o úroveň výš pro Velína).

Riziko k hlídání: tržby jsou citlivá data, platí stejné pravidlo jako všude — každé
číslo v UI musí mít dohledatelný zdroj (import → model → výpočet). Nerozhodnuto:
jestli externí Tourplan systém zůstane běžet paralelně (organizační/smluvní otázka,
ne technická) — ovlivní, jak rychle lze postoupit k C.
