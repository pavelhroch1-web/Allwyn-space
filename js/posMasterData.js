...
(function(global){
  ...
  function buildPosMasterMap(rawRows){
    ...
    const map = {};
    const warnings = [];
    let missingCoords = 0, missingHours = 0, duplicates = 0;

    rawRows.forEach((r) => {
      const posId = (r[COLS.POS_ID] || '').toString().trim();
      if (!posId) return;
      if (map[posId]) duplicates++;
    ...
    if (duplicates > 0) warnings.push(`${duplicates} duplicitních POS ID v master datech`);
    if (missingCoords > 0) warnings.push(`${missingCoords} POS bez platných GPS souřadnic — zůstane odhad podle adresy`);
    if (missingHours > 0) warnings.push(`${missingHours} POS bez otevírací doby — zůstane výchozí odhad 08:00-18:00`);

    const summary = {
      posCount: Object.keys(map).length,
      missingCoords, missingHours, duplicates,
    };

    return { map, summary, warnings };
  }

  // mergePosMasterData(pos, record) -> pos s doplněným GPS/openingHours
