...
3. **Deploy** — pokud Pavel řekl BUILD a testy prošly: commit na feature
   branch → merge do `main` → push `main` → GitHub Pages se nasadí
   automaticky → ověř živé URL, že ukazuje novou verzi (podle `CLAUDE.md`,
   DEPLOYMENT PRAVIDLO). Práce nesmí zůstat hotová jen na feature branch.
   Neříkej "hotovo", dokud veřejný GitHub Pages link neukazuje novou verzi —
   pokud nemůžeš nasadit (chybí oprávnění, blokující CI), řekni to otevřeně.
